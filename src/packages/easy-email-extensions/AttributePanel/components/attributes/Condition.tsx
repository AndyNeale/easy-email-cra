...
import {
  SelectField,
  TextField,
} from 'packages/easy-email-extensions/components/Form';
              ...
              <Grid.Row justify="space-between">
                <Grid.Col span={16}>
                  {condition.groups.length > 1 && (
                    <SelectField
                      inline
                      name={`${focusIdx}.data.value.condition.symbol`}
                      label={t('Symbol')}
                      options={[
                        {
                          label: t('And'),
                ...
                <Button
                  onClick={() =>
                    onAddCondition(`${focusIdx}.data.value.condition.groups`)
                  }
                  size="small"
                  icon={<IconPlus />}
                />
                  ...
                  <div>
                    <Grid.Row justify="space-between">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          onAddSubCondition(
                            `${focusIdx}.data.value.condition.groups.${gIndex}.groups`
                          )
                        }
                      />
                    </Grid.Row>
...
const options = Object.values(Operator).map(item => ({
...
function ConditionItem({
  ...
  const hideRight =
    value.operator === Operator.TRUTHY || value.operator === Operator.FALSY;

  return (
    <Grid.Row align="end">
      <Grid.Col span={7}>
        {' '}
        <TextField label={t('Variable path')} name={`${name}.left`} />
      </Grid.Col>
      <Grid.Col span={7}>
        {' '}
        <SelectField
      ...
      <Grid.Col span={7}>
        {' '}
        {!hideRight && <TextField label="Right" name={`${name}.right`} />}
      </Grid.Col>
      <Grid.Col span={3}>
        <Button
