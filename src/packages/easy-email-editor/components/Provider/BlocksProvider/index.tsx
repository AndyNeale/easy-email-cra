...
import { getPageIdx } from 'packages/easy-email-core';
import { EventManager } from 'packages/easy-email-editor';
import { EventType } from 'packages/easy-email-editor/utils/EventManager';
...
export const BlocksProvider: React.FC<{
  ...
  const onChangeTab: React.Dispatch<React.SetStateAction<ActiveTabKeys>> =
    useCallback(handler => {
      ...
      setActiveTab(currentTab => {
        const nextTab = handler as ActiveTabKeys;
        const next = EventManager.exec(EventType.ACTIVE_TAB_CHANGE, {
          currentTab,
          nextTab,
        });
